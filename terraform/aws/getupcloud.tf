    ...
    "Statement": [
        {
            ...
            "Resource": [
                "arn:aws:ec2:*:${data.aws_iam_user.aws_user.user_id}:volume/*",
                "arn:aws:ec2:*::snapshot/*",
                "arn:aws:s3:::${local.getupcloud_namespace_backup_aws_bucket_name}/*",
                "arn:aws:iam::${data.aws_iam_user.aws_user.user_id}:user/${aws_iam_user.getupcloud-namespace-backup.name}"
            ]
        },
        {
            "Effect": "Allow",
